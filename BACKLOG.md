...
- [x] **DOC-HW-001 · Documentation and README refresh (2026-09-10):** Updated all public project/status documents to the implemented UI, audio, accessibility, plan, translation, testing, and deployment state and replaced the README screenshot. No private release values, reviewer exports, or generated test artifacts were added.
...
- [ ] **UI-HW-002 · Phase-aware exercise overview and instructions:** Make each home-screen exercise card's phase clear through localized grouping or a phase label. Expose the localized exercise instructions through an accessible information control, using the same defined language fallback for custom exercises.
  - Support pointer hover, keyboard focus, touch/tap, and Escape instead of relying on a native `title` attribute. Give the trigger an accessible name, associate it programmatically with screen-reader-readable content, expose expanded state for a toggled popover, close without losing trigger focus, and keep content visible while pointer or focus remains within it.
  - Cover responsive layouts, long and right-to-left copy, repeated exercises in different phases, and custom exercises.
