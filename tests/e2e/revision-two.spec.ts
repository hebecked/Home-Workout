...
test('old feedback survives the revised review and only current decisions finish the round', async ({ page }) => {
  ...
  expect(reviews.filter(r => r.exerciseId === 'resistance-band-row')).toHaveLength(2);
  await page.evaluate((ids) => {
    const old = JSON.parse(localStorage.getItem('home-workout:illustration-reviews') ?? '[]') as unknown[];
    localStorage.setItem('home-workout:illustration-reviews', JSON.stringify([...old, ...ids.map(exerciseId => ({ exerciseId, revision: 3, status: 'confirmed', comment: '', reviewedAt: '2026-09-07' }))]));
