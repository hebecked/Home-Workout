  ...
  private shell(content: string): void {
    ...
    this.root.innerHTML = `
      <header class="site-header">
        <a href="#home" class="brand" aria-label="${escapeHtml(this.t('app.name'))}"><img class="brand-mark" src="/assets/branding/mountain-climber-header.png" alt="" aria-hidden="true" width="42" height="42"><span>${escapeHtml(this.t('app.name'))}</span></a>
    ...
    const previews = this.activePlan.phases.map((phase, phaseIndex) => {
      const cards = phase.exercises.map((exercise) => {
        ...
        return `<article class="exercise-preview-card">
          ...
          <img src="${definition?.illustration ?? '/mountain-climber-icon-192.png'}" alt="${escapeHtml(exerciseName)}" loading="eager">
        ...
        <div class="workout-content">
          ...
          ${snapshot.phase === 'completed' ? `<div class="completion"><p class="eyebrow">${escapeHtml(this.t('phase.complete'))}</p><h1>${escapeHtml(this.t('phase.complete'))}</h1><p>${escapeHtml(this.t('workout.completeCopy'))}</p><button class="primary" data-action="finish">${escapeHtml(this.t('button.backHome'))}</button></div>` : `
            ${isTransition ? `<div class="completion phase-transition-card"><p class="eyebrow">${escapeHtml(this.t('button.next'))}</p><h1>${escapeHtml(phaseLabel(this.activePlan.phases[snapshot.phaseIndex + 1]!.kind, this.t))}</h1><p>${escapeHtml(this.t('workout.restCopy'))}</p><strong data-workout-countdown>${formatClock(snapshot.remainingMs ?? 0)}</strong></div>` : `<div class="exercise-layout">
              <div class="exercise-visual-column"><div class="workout-exercise-heading">${displayNames.map(({ code, name }) => `<h2 lang="${escapeHtml(code)}">${escapeHtml(name)}</h2>`).join('')}</div><div class="exercise-visual"><img src="${definition?.illustration ?? '/mountain-climber-icon-192.png'}?v=${illustrationRevision(selectedExerciseId)}" alt="${escapeHtml(imageName)}"></div>
