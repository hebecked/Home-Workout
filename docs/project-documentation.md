...
Audio unit tests cover invalid, current, and legacy preferences, simplified persistence, deferred context creation, user-gesture resume behavior, tone scheduling, mute, and failure-safe behavior. Browser tests cover the opt-in default, quiet control placement, absence of a redundant slider, and in-workout mute.
